<script lang="ts">
	...
	async function changeTwitter() {
		...
		try {
			...
			if (!twitter) throw new Error('Vul een Twitter url in');
...
<form class="flex items-center justify-between gap-2" on:submit|preventDefault={changeTwitter}>
	<label for="twitter" class="flex w-8 justify-center"
		><Fa icon={faTwitter} class="text-3xl text-blue-400 mr-2" /></label
	>
	<div class="flex">
		<input
			id="twitter"
