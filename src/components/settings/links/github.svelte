<script lang="ts">
	...
	async function changeGithub() {
		...
		try {
			...
			if (!github) throw new Error('Vul een GitHub url in');
...
<form class="flex items-center justify-between gap-2" on:submit|preventDefault={changeGithub}>
	<label for="github" class="flex w-8 justify-center"
		><Fa icon={faGithub} class="text-3xl text-gray-800 mr-2" /></label
	>
	<div class="flex">
		<input
			id="github"
